  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 80; i++) {
      particles.push({
        ...
        color: Math.random() > 0.5 ? "167,139,250" : "99,102,241",
      });
    }
    let animId: number;
    const animate = () => {
...
export default function StreamPage() {
  ...
  useEffect(() => {
    ...
    const msgChannel = supabase.channel(`stream-chat-${streamId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter: `stream_id=eq.${streamId}` },
        (payload) => setChatMessages(prev => [...prev, payload.new]))
      .subscribe();
    const callChannel = supabase.channel(`call-requests-${streamId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "call_requests", filter: `stream_id=eq.${streamId}` },
        () => fetchCallRequests())
      .subscribe();
    return () => {
  ...
  const fetchMessages = async () => {
    const { data } = await supabase.from("messages").select("*").eq("stream_id", streamId).order("created_at", { ascending: true }).limit(50);
    if (data) setChatMessages(data);
  };

  const fetchCallRequests = async () => {
    const { data } = await supabase.from("call_requests").select("*").eq("stream_id", streamId).eq("status", "pending").order("created_at", { ascending: true });
    if (data) setCallRequests(data);
  };

  const sendMessage = async () => {
    if (!message.trim()) return;
    await supabase.from("messages").insert({ stream_id: streamId, user_name: userName, content: message });
    setMessage("");
  };

  const requestCall = async () => {
    await supabase.from("call_requests").insert({ stream_id: streamId, caller_name: userName, status: "pending" });
  ...
  const acceptCall = async (req: any) => {
    ...
    await supabase.from("messages").insert({ stream_id: streamId, user_name: "System", content: `🎉 ${req.caller_name} joined the stream!` });
  };
  ...
  const endCall = async () => {
    ...
    await supabase.from("messages").insert({ stream_id: streamId, user_name: "System", content: "📞 Call ended" });
  };
  ...
  return (
    <div className="h-screen text-white flex flex-col overflow-hidden" style={{background:"#060614"}}>
      <style>{`
        ...
        @keyframes neonPulse{0%,100%{box-shadow:0 0 20px rgba(124,58,237,0.4),0 0 40px rgba(124,58,237,0.2)}50%{box-shadow:0 0 30px rgba(124,58,237,0.7),0 0 60px rgba(124,58,237,0.4)}}
      ...
      <nav className="relative z-10 flex items-center justify-between px-5 py-3"
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full"
            ...
            <span className="text-violet-300 text-xs font-bold">VibeCity</span>
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0">
                  ...
                  <div className="text-center px-8">
                    ...
                    <div className="relative w-32 h-32 mx-auto mb-8 float">
                      ...
                      <div className="absolute -inset-1 rounded-full"
                        style={{background:"linear-gradient(135deg,#7c3aed40,#4f46e540)",border:"1px solid rgba(124,58,237,0.5)"}} />
                    ...
                    <p className="text-white/90 font-bold text-xl mb-2 tracking-tight">Evening Stream</p>
                    <p className="text-white/30 text-sm mb-8">
                      You're joining as{" "}
                      <span className="text-violet-400 font-semibold">{userName}</span>
                    </p>

                    <button onClick={joinStream} disabled={isLoading}
                      className="flex items-center gap-3 text-white font-bold px-10 py-4 rounded-2xl mx-auto disabled:opacity-40 transition-all hover:scale-105 text-base neon"
                      style={{background:"linear-gradient(135deg,#7c3aed,#4f46e5)"}}>
                      {isLoading ? (
                        <><span className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" />Connecting...</>
                      ) : (
                        <><Video size={20} />Join as Host</>
                      )}
                    </button>
              ...
              <div className="relative rounded-2xl overflow-hidden flex items-center justify-center"
                ...
                <div className="text-center relative z-10">
                  <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold mx-auto mb-3 text-white"
                    ...
                    {activeCaller[0].toUpperCase()}
                  </div>
                  <p className="text-white/80 text-sm font-semibold">{activeCaller}</p>
                  <p className="text-emerald-400/60 text-xs mt-1">Live Guest</p>
                </div>
          ...
          <div className="flex items-center justify-center gap-3 px-4 py-3.5 flex-wrap flex-shrink-0"
            ...
            <button onClick={toggleMic} className="ctrl-btn"
              style={{
                background: micOn ? "rgba(124,58,237,0.12)" : "rgba(239,68,68,0.4)",
            ...
            <button onClick={toggleCam} className="ctrl-btn"
              style={{
                background: camOn ? "rgba(124,58,237,0.12)" : "rgba(239,68,68,0.4)",
            ...
            <button onClick={() => { setLiked(!liked); setLikeCount(p => liked ? p-1 : p+1); }}
              className="ctrl-btn"
              style={{
                background: liked ? "rgba(236,72,153,0.35)" : "rgba(124,58,237,0.12)",
            ...
            {isHost ? (
              <button onClick={() => setShowCallRequests(!showCallRequests)}
                className="relative flex items-center gap-2 text-white font-bold px-5 py-2.5 rounded-2xl text-sm transition-all hover:scale-105"
              ...
              <button onClick={requestCall}
                className="flex items-center gap-2 text-white font-bold px-5 py-2.5 rounded-2xl text-sm transition-all hover:scale-105 neon"
                style={{background:"linear-gradient(135deg,#7c3aed,#4f46e5)"}}>
                <Phone size={15} />
                Call Into Stream
              </button>
                ...
                {callRequests.map((req) => (
                  <div key={req.id} className="flex items-center gap-3 p-3 rounded-2xl"
                    ...
                    <button onClick={() => rejectCall(req)}
                      className="text-white/40 text-xs px-3 py-1.5 rounded-xl font-medium transition-colors hover:text-white"
